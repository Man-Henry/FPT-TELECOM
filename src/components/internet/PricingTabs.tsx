    ...
    <section className="bg-background py-24">
      <div className="container mx-auto px-4 md:px-6">
        <div className="mb-12 text-center">
          <h2 className="text-foreground mb-4 text-3xl font-bold tracking-tight md:text-4xl">Bảng Giá Cước</h2>
          <p className="text-muted-foreground mx-auto max-w-2xl text-lg">Chọn gói cước phù hợp nhất.</p>
        </div>
        ...
        <Tabs defaultValue="ca-nhan" className="mx-auto w-full max-w-5xl">
          <TabsList className="mx-auto mb-12 grid h-12 w-full max-w-md grid-cols-2 rounded-lg bg-white/[0.04] p-1">
            <TabsTrigger value="ca-nhan" className="h-full rounded-md text-sm font-semibold data-[state=active]:bg-white/[0.08] data-[state=active]:text-white">
              Cá nhân / Gia đình
            </TabsTrigger>
            <TabsTrigger value="doanh-nghiep" className="h-full rounded-md text-sm font-semibold data-[state=active]:bg-white/[0.08] data-[state=active]:text-white">
              Doanh nghiệp
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="ca-nhan" className="animate-in fade-in-50 mt-0 duration-300">
            <div className="rounded-xl border border-white/[0.06] bg-[#0C1524] p-6 md:p-8">
              <div className="mb-6 flex items-center gap-3">
                <div className="rounded-lg bg-white/[0.04] p-2 text-slate-400"><Router className="h-5 w-5" /></div>
                <div>
                  <h3 className="text-lg font-bold">Gói cước Gia Đình</h3>
                  <p className="text-muted-foreground text-sm">Phù hợp nhu cầu lướt web, học tập, giải trí.</p>
                </div>
              </div>
              <div className="mt-6"><BestSellerTable /></div>
              ...
              <div className="mt-6 flex flex-col items-center justify-between rounded-lg border border-white/[0.06] bg-white/[0.02] p-4 md:flex-row">
                <div className="mb-4 flex items-start gap-3 md:mb-0">
                  ...
                  <p className="text-muted-foreground text-sm">
                    <strong className="text-foreground">Ưu đãi:</strong> Trả trước 6 tháng tặng 1 tháng, 12 tháng tặng 2 tháng. Phí hòa mạng: 300.000đ. Mesh/Modem lắp thêm: +20.000đ/tháng.
                  </p>
                </div>
                <a href="#dang-ky" className={cn(buttonVariants({ variant: 'default' }), 'bg-primary text-primary-foreground whitespace-nowrap hover:bg-primary/90')}>
                  Đăng Ký Ngay
                </a>
              </div>
            </div>
          ...
          <TabsContent value="doanh-nghiep" className="animate-in fade-in-50 mt-0 duration-300">
              ...
              {businessPlans.map((plan, index) => (
                <div key={index} className={cn(
                  ...
                  <div className="mb-6">
                    ...
                    <div className="text-primary mb-3 inline-block rounded bg-white/[0.04] px-2 py-0.5 text-sm font-medium">{plan.speed}</div>
            ...
            <div className="mt-6 rounded-xl border border-white/[0.06] bg-[#0C1524] p-5 text-center">
              <p className="text-muted-foreground mb-3 text-sm">Doanh nghiệp lớn cần băng thông cam kết?</p>
              <Button variant="outline" className="rounded-lg border-white/[0.06] text-slate-300 hover:bg-white/[0.04]">
                Nhận Báo Giá
              </Button>
            </div>
          </TabsContent>
