  ...
  return (
    <section className="relative overflow-hidden bg-[#0A1128] pt-32 pb-20 text-white md:pt-40 md:pb-28">
      <div className="absolute inset-0 z-0">
        ...
        <Image src="/images/hero-combo-fpt.jpg" alt="Gói Combo FPT" fill className="object-cover opacity-20" priority />
      </div>
      ...
      <div className="relative z-20 container mx-auto px-4 md:px-6">
        <Breadcrumb className="mb-8">
          <BreadcrumbList className="text-slate-500">
            <BreadcrumbItem>
              <BreadcrumbLink href="/" className="flex items-center transition-colors hover:text-slate-300"><Home className="mr-1 h-4 w-4" />Trang chủ</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator className="text-slate-600" />
            <BreadcrumbItem><BreadcrumbPage className="font-semibold text-white">Gói Combo</BreadcrumbPage></BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>

        <div className="max-w-3xl">
          <div className="text-primary mb-6 inline-flex items-center rounded-md bg-white/[0.04] px-3 py-1 text-sm font-medium">
            <Package className="mr-2 h-4 w-4" />Tiện Ích All-in-One
          </div>

          <h1 className="mb-6 text-4xl leading-tight font-extrabold tracking-tight md:text-5xl lg:text-6xl">
            Combo Internet + Truyền Hình{' '}
            <span className="text-primary">Siêu Tiết Kiệm</span>
          </h1>

          <p className="text-muted-foreground mb-8 max-w-2xl text-lg md:text-xl">
            Đăng ký một lần - Tận hưởng đa tiện ích. Gộp chung trên cùng một hoá đơn, tiết kiệm lên đến 30%.
          </p>

          <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:gap-8">
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-white/[0.04] p-2.5 text-slate-400"><Percent className="h-5 w-5" /></div>
              <div className="flex flex-col">
                <span className="font-semibold text-white">Rẻ Hơn 30%</span>
                <span className="text-sm text-slate-500">So với đăng ký lẻ</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="rounded-lg bg-white/[0.04] p-2.5 text-slate-400"><HeartHandshake className="h-5 w-5" /></div>
              <div className="flex flex-col">
                <span className="font-semibold text-white">Một Hoá Đơn</span>
                <span className="text-sm text-slate-500">Dễ dàng quản lý</span>
              </div>
            </div>
          </div>
        </div>
